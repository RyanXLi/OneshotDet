...
def do_coco_evaluation(
    ...
    if box_only:
        ...
        for limit in [100, 1000, 4000, 5000, 8000, 10000]:
...
def prepare_for_coco_detection(predictions, dataset):
    ...
    import json
    # assert isinstance(dataset, COCODataset)
    # cocoRef = COCO('datasets/coco/annotations/instances_val2017.json')
    # cocoRef = COCO('/data/xlide/datasets/voc/VOC2012/Annotations/pascal_test2007.json')
    cocoRef = COCO('/data/linz/few_shot/fcos_plus/fcos_plus/datasets/ILSVRC2014/ILSVRC2014_train.json')
    # cocoRef = COCO('/data/linz/few_shot/fcos_plus/fcos_plus/datasets/voc_2010/VOCdevkit/VOC2010/voc2010_instances_all.json')
    coco_custom_gt = {
    "info": {
    ...
    for image_id, prediction in enumerate(predictions):
        ...
        img_info, img_cur_cat = dataset.get_img_info(image_id)

        image_width = img_info["width"]
        image_height = img_info["height"]

        img_info_copy = copy.copy(img_info)
        img_info_copy['id'] = image_id

        coco_custom_gt['images'].append(img_info_copy)
        ann_ids = cocoRef.getAnnIds(imgIds=original_id, catIds=img_cur_cat, iscrowd=False)
        
        if not ann_ids:
            print(image_id)
            print('no ann_ids, continue')
            continue
        anno = cocoRef.loadAnns(ann_ids)

        for item in anno:
            item['image_id'] = image_id # add
            item["category_id"] = img_cur_cat
            coco_custom_gt['annotations'].append(item)

        coco_ids.append(original_id)
        ...
        labels = [1 for item in scores]

        mapped_labels = labels#[dataset.contiguous_category_id_to_json_id[i] for i in labels]

        coco_results.extend(
            [
                {
                    "image_id": image_id,#original_id,
                    "category_id": img_cur_cat,#mapped_labels[k],
                    "bbox": box,
                    "score": scores[k],
                }
                for k, box in enumerate(boxes) #if mapped_labels[k] == 1
            ]
        )

    save_custom_gt_path = "coco_custom_gt.json"
    with open(save_custom_gt_path, "w") as f:
        json.dump(coco_custom_gt, f, sort_keys=True, indent=4, separators=(',', ':'))
    print("Save coco format custom gt to {}".format(save_custom_gt_path))

    save_custom_result_path = "coco_custom_result.json"
    with open(save_custom_result_path, "w") as f:
        json.dump(coco_results, f, sort_keys=True, indent=4, separators=(',', ':'))
    print("Save coco format custom result to {}".format(save_custom_result_path))

    save_ids_path = "coco_ids.json"
    with open(save_ids_path, "w") as f:
        json.dump(coco_ids, f, sort_keys=True, indent=4, separators=(',', ':'))
    print("Save coco ids result to {}".format(save_ids_path))
...
def evaluate_box_proposals(
    ...
    for image_id, prediction in enumerate(predictions):
        ...
        #added
        img_info, img_cur_cat = dataset.get_img_info(image_id)
        ann_ids = dataset.coco.getAnnIds(imgIds=original_id, catIds=img_cur_cat)
    ...
    if thresholds is None:
        ...
        thresholds = torch.arange(0.5, 0.5 + 1e-5, step, dtype=torch.float32)
...
def evaluate_predictions_on_coco(
    ...
    coco_gt = COCO("coco_custom_gt.json")
